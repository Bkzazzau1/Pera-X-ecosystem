use crate::{
    ...
    ExecuteSettlementMarketPurchase, ExecuteSettlementMarketPurchaseParams,
    ExecuteSettlementVaultFunding, ExecuteSettlementVaultFundingParams, FinalizeSettlement,
    FinalizeSettlementParams, FundDirectPexSettlement, FundDirectPexSettlementParams,
    InitializeProductSettlementPolicy, InitializeProductSettlementPolicyParams,
    InitializeSettlementPolicy, InitializeSettlementPolicyParams, PlanSettlement,
    PlanSettlementParams, ProductSettlementPolicyInitialized, ProductSettlementPolicyUpdated,
...
pub fn initialize_settlement_policy(
    ...
    policy.settlement_authority = ctx.accounts.settlement_authority.key();
    policy.settlement_pex_vault = ctx.accounts.settlement_pex_vault.key();
    ...
    policy.settlement_authority_bump = ctx.bumps.settlement_authority;

    emit!(SettlementPolicyInitialized {
        settlement_policy: policy.key(),
        ...
        settlement_pex_vault: policy.settlement_pex_vault,
...
pub fn plan_settlement(ctx: Context<PlanSettlement>, params: PlanSettlementParams) -> Result<()> {
    ...
    emit!(SettlementPlanned {
        ...
        pex_obligation: record.pex_obligation,
        market_pex_required: record.market_pex_required,
        policy_vault_pex_required: record.policy_vault_pex_required,
        planned_at: now,
    });
    Ok(())
}
...
pub fn fund_direct_pex_settlement(
    ...
    record.direct_pex_received = record
        ...
        .ok_or(SettlementError::SettlementArithmeticError)?;
    record.funding_source_token_account = set_or_validate_funding_source(
...
pub fn execute_settlement_market_purchase<'info>(
    ...
    record.funding_source_token_account = set_or_validate_funding_source(
        record.funding_source_token_account,
        ctx.accounts.quote_source_token_account.key(),
    )?;
    refresh_settlement_status(record)?;

    emit!(SettlementMarketPurchaseExecuted {
        settlement_record: record.key(),
        quote_source_token_account: ctx.accounts.quote_source_token_account.key(),
        quote_spent,
        pex_received,
        executed_at: now,
    });
...
pub fn execute_settlement_vault_funding(
    ...
    emit!(SettlementPolicyVaultFunded {
        settlement_record: record.key(),
        reserve_vault_config: ctx.accounts.reserve_vault_config.key(),
...
pub fn finalize_settlement(
    ...
    match ctx.accounts.settlement_record.disposition {
        ...
        SettlementDisposition::Lock => require!(
            ctx.accounts.destination_token_account.key() == ctx.accounts.lock_vault.key(),
            SettlementError::InvalidSettlementDestination
        ),
        SettlementDisposition::Burn => require!(
            ctx.accounts.destination_token_account.key() == ctx.accounts.lock_vault.key(),
            SettlementError::InvalidSettlementDestination
        ),
    }

    let bump = [ctx.accounts.settlement_policy.settlement_authority_bump];
    let settlement_policy_key = ctx.accounts.settlement_policy.key();
    let signer_seeds: &[&[u8]] = &[
        b"settlement-authority",
        settlement_policy_key.as_ref(),
        &bump,
    ];
    ...
    match ctx.accounts.settlement_record.disposition {
        SettlementDisposition::Burn => {
            ...
            if surplus > 0 {
                transfer_from_settlement_vault(
                    &ctx,
...
fn transfer_from_settlement_vault<'info>(
    ctx: &Context<FinalizeSettlement<'info>>,
